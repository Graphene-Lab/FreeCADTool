...
- AgentBridge Chat workbench: the C# auto-install (`EnsureChatMod`/`InstallChatMod`), the nupkg `chat_mod` packing, the Qt import shim (`_qt.py`), the headless import safety of `InitGui.py`, and the SSE chat-client contract were all verified (import smoke-tests pass under `freecadcmd` on FreeCAD 0.20.2 and 1.1.3; the SSE client was checked against a mock streaming server). The **full GUI round-trip** — the dock actually rendering in a desktop FreeCAD, the bridge starting inside the visible GUI instance, and the agent editing that window live — requires a real desktop FreeCAD GUI plus a running AgentBridge and was **not** executed in this headless environment. It should be confirmed manually on a desktop before relying on the chat in production. The `commands.py` cross-process-detection gap (see MAINTENANCE.md) is handled by the in-process `_bridge_started` guard, so a double `_open_chat()` cannot double-bind the ports.
